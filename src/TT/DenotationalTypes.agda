module TT.DenotationalTypes where
...
elimDesc : (P : (ℓ : ℕ) → Desc ℓ → Set)
  (p⊤ : (ℓ : ℕ) → P ℓ `⊤)
  (pX : (ℓ : ℕ) → P ℓ `X)
  (pΠ : (ℓ : ℕ) (A : Type ℓ) (D : ⟦ ℓ ∣ A ⟧ → Desc (suc ℓ))
    (rec : (a : ⟦ ℓ ∣ A ⟧) → P (suc ℓ) (D a))
    → P (suc ℓ) (`Π A D))
  (pΣ : (ℓ : ℕ) (A : Type ℓ) (D : ⟦ ℓ ∣ A ⟧ → Desc (suc ℓ))
    (rec : (a : ⟦ ℓ ∣ A ⟧) → P (suc ℓ) (D a))
    → P (suc ℓ) (`Σ A D))
  → (ℓ : ℕ) (D : Desc ℓ) → P ℓ D
...
elimType : (P : (ℓ : ℕ) → Type ℓ → Set)
  ...
  (pDesc : (ℓ : ℕ) → P ℓ `Desc)
  (pType : (ℓ : ℕ) → P ℓ `Type)
  (pμ : (ℓ : ℕ) (D : Desc ℓ) → P ℓ (`μ D))
  (pΠ : (ℓ : ℕ) (A : Type ℓ) (B : ⟦ ℓ ∣ A ⟧ → Type ℓ)
    (rec₁ : P ℓ A)
    (rec₂ : (a : ⟦ ℓ ∣ A ⟧) → P ℓ (B a))
    → P ℓ (`Π A B))
  (pΣ : (ℓ : ℕ) (A : Type ℓ) (B : ⟦ ℓ ∣ A ⟧ → Type ℓ)
    (rec₁ : P ℓ A)
    (rec₂ : (a : ⟦ ℓ ∣ A ⟧) → P ℓ (B a))
    → P ℓ (`Σ A B))
  (p⟦D⟧ᵈ : (ℓ : ℕ) (D : Desc ℓ) (X : Type ℓ) (rec : P ℓ X) → P ℓ (`⟦ D ⟧ᵈ X))
  (p⟦A⟧ : (ℓ : ℕ) (A : Type ℓ) (rec : P ℓ A) → P (suc ℓ) `⟦ A ⟧)
  → (ℓ : ℕ) (A : Type ℓ) → P ℓ A

elimType P p⊥ p⊤ pBool pℕ pDesc pType pμ pΠ pΣ p⟦D⟧ᵈ p⟦A⟧
  ℓ `⊥ = p⊥ ℓ
elimType P p⊥ p⊤ pBool pℕ pDesc pType pμ pΠ pΣ p⟦D⟧ᵈ p⟦A⟧
  ℓ `⊤ = p⊤ ℓ
elimType P p⊥ p⊤ pBool pℕ pDesc pType pμ pΠ pΣ p⟦D⟧ᵈ p⟦A⟧
...
elimType P p⊥ p⊤ pBool pℕ pDesc pType pμ pΠ pΣ p⟦D⟧ᵈ p⟦A⟧
  ℓ (`Π A B) =
  let f = elimType P p⊥ p⊤ pBool pℕ pDesc pType pμ pΠ pΣ p⟦D⟧ᵈ p⟦A⟧ ℓ
